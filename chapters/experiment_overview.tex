...
\Cref{tab:btosgamma_bfs} provides the experimental status of the most prominent observed \BtoXsdgamma decay channels as of writing this.
...
\begin{figure}[hbtp!]
    ...
    Photon energy spectra in \BtoXsgamma decays before (\subref{fig:untagged_btosgamma_background}) and after (\subref{fig:untagged_btosgamma_measured}) background suppression.
    ...
    In (\subref{fig:untagged_btosgamma_measured}), official Belle~II data from Ref.~\cite{Collaboration:2302} is shown.
    \Cref{fig:untagged_btosgamma_background} is produced for illustrative purposes only. 
    }
\end{figure}
...
\begin{figure}[hbtp!]
    ...
    \subcaptionbox{\label{fig:tagging_advantages}}{
        \begin{tikzpicture}
            \node[anchor = north west, inner sep=0] (image) at (current page.north west) 
                {    \includegraphics[width=0.5\textwidth]{figures/experiment_overview/tagging_advantages_mod.png}};
            \node[align = center, below = 1mm of image] (firsttext)% <---- 
                {\scriptsize \textbf{Untagged} \tab \textbf{lepton-tagging} \tab \textbf{semileptonic-tagging} \tab \textbf{hadronic-tagging}};
            % \node[align = center, below = 1mm of firsttext] (sectext)% <---- 
            %     {\scriptsize $B\to\mathrm{anything}$ \tab $B\to D^(*)\ell\nu \mathrm{n}\cdot\pi$ \tab$B\to\mathrm{hadrons}$   };
        \end{tikzpicture}
    }
    \caption{\label{fig:tagging_drawins} Schematic representation of tagging is shown in (\subref{fig:tagging_schematic}).
    ...
    In (\subref{fig:tagging_advantages}), the advantages and disadvantages related to using different $B$ decay products for tagging at $B$-factories are highlighted.
...
The ability to rely on these distributions and, in particular, perform a signal extraction fit, similar to the one given in \Cref{fig:mbc_sum_of_exclusive}, allows suppressing previously dominant $\epem\ra\qqbar$ components to negligible levels.

Because both $B$ mesons at $B$-factories are created from decays of \FourS, 
