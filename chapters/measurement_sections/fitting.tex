...
\begin{figure}[htbp!]
    ...
    The final binning chosen is a hybrid scenario described in the text of \Cref{sec:binning}.
...
The Figures related to the primary fits are shown in \Cref{sec:appendix_primary_fits}.
...
The primary Crystal Ball fits for every \EB interval are shown in \Cref{fig:primary_cb_fits}.
...
The primary Chebyshev fits for every \EB interval are shown in \Cref{fig:primary_cheb_fits}.
...
The primary ARGUS fits for every \EB interval are shown in \Cref{fig:primary_argus_fits}.
...
This fitting model is then applied to the total data set in \Cref{sec:mbc_fit_full_mc} 
...
The prepared fitter, discussed in \Cref{sec:fitting_setup}, is applied to the total generic \MC data set.
...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:primary_full_fits}The fits on the $1.6~\invab$ data set of generic \MC, using the fitting model from \Cref{tab:fitting_init_params}.
    A good description of the \Mbc distributions can be seen throughout the \EB bins.
    This highlights that the fit setup accurately describes the simulated data set that was used to initialise it.
    }
\end{figure}
