...
Then, some fitting parameters are grouped together, such that a simultaneous determination with some parameters shared across multiple bins is prepared.
...
\begin{table}[htbp!]
    ...
    The table lists only the shape parameters and not the normalisation parameter $\mathcal{N}$, which is also determined for every shape and every \EB bin.
    }
