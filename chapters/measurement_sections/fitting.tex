...
Extracted from generic \MC, they are visualised \Cref{fig:tag_component_fits}.
...
It is clear from \Cref{fig:spectrum_after_optimisation} that the signal-to-background ratio changes across all \EB bins.
...
On the other hand, the studies performed in \Cref{sec:resolution_studies} show that the resolution of \EB is around $40~\mev$.
...
The primary fit results are presented in \Cref{tab:fitting_init_params}.
...
\begin{figure}[hbtp!]
    ...
    Additionally, results are shown for the fits of the generic \MC data set but with all \BtoXsgamma events removed (\subref{fig:no_bxsgamma_mc_fit_yield_comparisons}),
