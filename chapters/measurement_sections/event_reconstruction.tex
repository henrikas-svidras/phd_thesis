...
\end{table}
The requirement to have at least 3 clean tracks in the event and at least 3 clean \ECL clusters is based on the fact, that a hadronic day will produce many charged tracks and photons.
Furthermore, measured energy in the event in the \epem collision centre-of-mass frame is required to exceed 4~\gev.
...
\begin{table}[htbp!]
    \centering
     ...
     In this analysis, only the selection on the tag-side \B meson is tightened in order to remove the edge effects near the 5.24~\gev cutoff point.
...
Reconstructed photon energy is boosted to the candidate $B$ meson rest frame based on the Lorentz transformation in \Cref{sec:appendix_boosting_to_b_frame}.
...
The reconstructed \BtoXsgamma spectra in generic \MC with the previously laid-out requirements are shown in \Cref{fig:spectrum_after_reco}.
...
However, one has to take into account that in general, \feiBp and \feiBz modes result
