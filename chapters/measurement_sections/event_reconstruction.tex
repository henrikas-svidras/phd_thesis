...
Using the definitions of cleaned tracks and \ECL clusters, reconstructed events are filtered and only the events that pass these requirements are analysed by the \FEI algorithm.
...
On the other hand, comparing \Cref{fig:untagged_btosgamma_background,fig:spectrum_after_reco} it is clear that a better signal-to-background ratio can already be observed even without any additional background treatment.
