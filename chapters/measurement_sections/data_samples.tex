...
For \BtoXsgamma, given the experimental resolution, as well as the fact that many different decay resonances combine and interfere this is an excellent approximation.
...
\begin{figure}[htbp!]
    ...
    The maximum deviations (envelope) are taken as modelling uncertainty.}
\end{figure}
...
In each \EB bin, an appropriate hybrid weight is calculated, such that the sum of the reweighted inclusive model (\Cref{fig:inclusive_reweighted}) and $\B\to\Kstar(892)\gamma$ contribution matches the partial branching fraction within that \EB bin, based on \Cref{eq:hybrid_model_definition}.
...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:hybrid_bins_and_model} Hybrid model constructed for this analysis 
    ...
    It is clear, that the hybrid model describes the tail adequately, 
