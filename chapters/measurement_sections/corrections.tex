...
The real challenge, as usual, is ensuring that the conclusions and results observed in \MC will generalise correctly to real Belle II data.
...
\begin{figure}[htbp!]
    ...
    Figures are taken from \cite{Belle-II:2020fst}.
    }
\end{figure}
...
Therefore, a linear extrapolation is performed to estimate the value for values outside the range.
It is observed that the linear extrapolation is consistent, within errors, with the corrections in the $1.8-3.0~\gev$.
...
The presence of neutrinos creates a missing momentum which can fake that originating from an \ISR photon.
...
Finally, instances, where two or more \ISR events occur, can complicate the photon finding procedure, resulting in an overall drop in efficiency.
...
The finding efficiencies are calculated and their ratio in data and simulation is shown.
In \MC, only $\epem\ra\mumu$ events are used for the finding efficiency.
The measured backgrounds, particularly the most prominent $\epem\ra\tautau$ background, are subtracted from the data distributions.
...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:data_mc_photon_eff} The photon finding efficiency as a function of missing momentum of the dimuon system, \pRecoil.
    The subpanel shows the ratio of data and simulation finding efficiencies, as per \Cref{eq:photon_data_mc}.
    All figures include systematic uncertainties, which are correlated between data and simulation.
    The figures show results for the datasets of a size equivalent to the ones used in this analysis.
    }
\end{figure}
...
Although a full study of all possible background modes that may contribute to the leftover-\BB background is, likely, a subject for another full doctoral thesis, 
...
The table also contains their relative fraction in every given bin.
As hundreds of $B$ decay modes contribute at $\order(<1\%)$ relative fraction, these requirements may not cover all backgrounds in low-\EB regions. 
...
The main sources of background photons are various $B\to D$ transitions where photons originate from either subsequent $D$ decays or the accompanying particle, e.g. $B\to D\rho$.
...
The results are summed together and their uncertainties are propagated, taking the correlations, resulting from the fact that the same decay mode may contribute in multiple \EB bins.
...
Although no special corrections are calculated, an additional selection is added to ensure that photons produced by the beam background
...
Such machine-related backgrounds vary based on the experimental conditions and it is known that these effects are not well captured by the Belle~II run-period independent simulation that was used in preparation for this analysis.
