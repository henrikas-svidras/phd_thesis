...
\begin{figure}[hbtp!]
    ...
    $\varepsilon_{\mathrm{FEI}}$, shown in (\subref{fig:epsilon_fei}), is seen to vary lightly, no more than 10\% accross the $\EtildeB$ range.
    $\varepsilon_{\mathrm{selection}}$, shown in (\subref{fig:epsilon_selection}) for three different models,
...
The variation is evaluated at \mbox{$\Delta\varepsilon\approx0.024\pm0.043$}.
For a conservative approach, a 3\% efficiency uncertainty is adopted, associated with the \texttt{BDT~output} modelling.

The results of this Section, \Cref{sec:signal_efficiency} and corrections from \Cref{sec:corrections}
are combined in \Cref{eq:signal_efficiency} to calculate the selection efficiency of \BtoXsgamma as a function of \EB.
...
The fits are done with two choices of tag-\B mesons: firstly, with the good tag-\B mesons as defined in \Cref{sec:good_tag_definition},
...
\begin{figure}[hbtp!]
    ...
    and the measured photon energy (\EB) is seen in (\subref{fig:btosgamma_resolution}).
    The spectrum is slightly shifted to lower energies after the measurement.
    A corresponding response matrix is built for these distributions and shown in (\subref{fig:migration_matrix}).
