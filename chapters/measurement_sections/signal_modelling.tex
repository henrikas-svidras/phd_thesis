...
Combined with hte average \FEI tagging efficiency (given in \Cref{eq:avg_efficiency_fei}) using the factorised relation in \Cref{eq:factorisable_signal_efficiency}, these values form the final
...
Based on \Cref{eq:effective_lagrangian}, neglecting corrections, the $\BtoXdgamma$ branching fraction
is suppressed by \cite{Workman:2022ynf}:
\todo[inline]{not pdg here cite smth else}
...
This can be see in \Cref{fig:btosgamma_resolution}, where the reconstructed and measured photon energies are compared.
...
As it was discussed in \Cref{sec:unfolding}, in this analysis the bin-by-bin correction method is used for unfolding.
...
The measurement uncerainties are evaluated as the average uncertainty from toy fits in \Cref{sec:background_subtraction_validation_mc}, scaled for lower luminosity, and corrected for background and signal modelling based on discussions in \Cref{sec:corrections,sec:signal_modelling}.
...
\begin{figure}[htbp!]
    ...
    The shaded area represenets the expected measurement uncertainties, based on the toy study in \Cref{sec:background_subtraction_validation_mc}.
