...
The full analysis procedure on simulation is defined and produces an unbiased, stable result in \Cref{sec:MC_validation}.
...
The \Mbc fits on Belle~II \MC and data are shown in \Cref{fig:mbc_bbar_ehnhanced_fits_mc,fig:mbc_bbar_ehnhanced_fits_data}, respectively.
...
Correcting the simulation based on \Cref{tab:correction_table} (with inverted \piVeto and \etaVeto correction as reported in \Cref{eq:correction_transform}) and calculating the difference between the two is expected to yield a value consistent with zero.
...
\begin{figure}[htbp!]
    ...
    from that in Belle~II data (\Cref{fig:mbc_bbar_ehnhanced_fits_data}) in the \BB-background enhanced samples (see \Cref{sec:bb_background_validation}).
