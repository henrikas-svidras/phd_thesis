...
due to the a small number of \BtoXsgamma events and a low signal-to-background ratio expected there.
These same arguments, however, make the regions excellent for background validation.

The \EB distribution, for the 3 \EB sideband-intervals are shown in \Cref{fig:sidebands_eb}.
...
While some discerapncy is expected considering the results of \Cref{fig:qqbar_enhanced_mbccorrected},
...
Although more studies on this subject are necessary to fully understand the discrepancy, it is attributed to a continuum-like component which is not well modelled in Belle~II simulation, the potential origin of which is shortly discussed here.
...
These observations strongly suppor the previous hypothesis that a hadronic cluster, perhaps particularly common to \epem\ra\qqbar events as a photon candidate, is not well-modelled in simulation.
As a result, this component is suppressed in Belle~II simulation by a selection on the \texttt{zernikeMVA} observable, but not in Belle~II data.
\begin{figure}[htbp!]
...
These results are also conflated with the differences in \Mbc endpoint and are therefore at least partially correlated, making an exact evaluation of the effect difficult at this stage.
...
This may imply that this type of selection is particular for particles from non-\BB events misidentified as high-energy photons.
Therefore, it was concluded that additional studies of \ZMVA in the context of radiative and inclusive analyses will be necessary.
...
The fit is performed on a hybrid data-simulation sample, where data is used in $\EB\in(1.4,1.8)~\gev$ and $2.7<\EB$~\gev regions, but the rest of the \EB-regions are represented by randomly sampled $189~invfb$ of Belle~II simulation.
