...
Such data preprocessing will be performed based on the variables described in \Cref{sec:photon_selection}.
...
\begin{figure}[htbp!]
    ...
    \subcaptionbox{\label{fig:bp_zmva_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bp_zernikeMVA_optimisation_punzi.pdf}
        }
    \subcaptionbox{\label{fig:bp_piveto_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bp_piVeto_optimisation_punzi.pdf}
        }
    \subcaptionbox{\label{fig:bp_etaveto_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bp_etaVeto_optimisation_punzi.pdf}
        }
    \subcaptionbox{\label{fig:bz_zmva_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bz_zernikeMVA_optimisation_punzi.pdf}
        }
    \subcaptionbox{\label{fig:bz_piveto_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bz_piVeto_optimisation_punzi.pdf}
        }
    \subcaptionbox{\label{fig:bz_etaveto_optimisation}}{
            \includegraphics[width=0.30\textwidth]{figures/continuum_suppression/Bz_etaVeto_optimisation_punzi.pdf}
        }
...
Based on \Cref{fig:selection_optimisations}, pre-selections are chosen to suppress background but retain most of the signal.
...
Belle~II has a large selection of observables that can be utilised for continuum suppression that are suitable to be input as features to a \BDT.
...
\begin{itemize}
    ...
    The metric is required to be no more than $0.1$.
    The looser requirement is adopted here because some difference is expected between the distributions:
    the collision energy in the off-resonance data set is different to the one in the on-resonance simulation.
...
The tested distributions include the selections from \Cref{sec:preselection}, except for the case of the off-resonance dataset, where the $\Mbc>5.27~\gevcc$ requirement is lifted.
For every event, when more than one tag-side candidate $B$ candidate exists, a random one is picked.
The application of \textbf{Test~1} with exact definitions for the 75 observables are shown in \Cref{sec:appendix_continuum_features}.
...
For signal, half is taken from the $\BptoXsgamma$ signal mode, the other half from the $B^0$, irrespective of the \FEI mode.
...
The resulting $\mathtt{BDT~output}$ is tested to not bias the photon energy spectrum to further ensure the validity of the training.
...
\begin{figure}[htbp!]
    ...
    In \Cref{fig:bdt_output_correlation}, correlation tests for the trained classifier output described in \Cref{sec:continuum_training} are depicted as a 2D histogram.
