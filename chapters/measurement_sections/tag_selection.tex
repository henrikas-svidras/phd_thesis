...
\begin{figure}[htbp!]
    ...
    and for \BztoXsgamma in \Cref{fig:bz_same_mode_best_tag_selection}.
...
\begin{figure}[htbp!]
    ...
    and \mbox{\epem\ra\qqbar} events from generic \MC (\Cref{fig:bp_continuum_mode_best_tag_selection,fig:bz_continuum_mode_best_tag_selection}) after selecting a single tag-$B$ meson per event either randomly or by requiring the largest \feiProb.
    \Cref{fig:bp_mbc_mode_best_tag_selection,fig:bz_mbc_mode_best_tag_selection} show that the difference in the \Mbc distribution for \BptoXsgamma and \BztoXsgamma mostly pronounced in the peak region.
    On the other hand, \Cref{fig:bp_continuum_mode_best_tag_selection,fig:bz_continuum_mode_best_tag_selection} show no strong dependence in shape irrespective of the way the tag-side candidate is chosen.
    The Figures are normalised to their total integral value such that a shape comparison can be performed.
...
In particle physics simulation it is possible to rely on the information provided by the simulation generators to trace back the measured and reconstructed particles to the generated particles.
...
The modes chosen in this analysis as ones that provide good kinematic constraints are summarised in \Cref{tab:error_codes}.
...
The \Mbc distributions for \BtoXsgamma the admixture with categories providing a `good' kinematic constraint and a `bad' one are shown in \Cref{fig:goodbad_comparison}.
...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:good_tag_definitions} \Mbc distributions of \BtoXsgamma admixture with split by `good'/`bad' kinematic constrain criterion (\Cref{fig:goodbad_comparison})
    and the conventional perfect/imperfect reconstruction criterion (\Cref{fig:isSig_comparison}).
