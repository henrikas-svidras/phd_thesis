...
The subsequent collision occurs in the interaction region inside the Belle~II detector, where the electron and positron rings meet (see \Cref{sec:belle2}).
\begin{figure}[hbtp!]
    \centering
...
This is shown for \mbox{$\sqrt{s}\approx10.58~\gev$} in \Cref{fig:cross_sections}, with more details about the exact values of the cross-sections provided in \Cref{sec:appendix_major_production_cross_sections}.
\begin{figure}[hbtp!]
...
such as \mbox{$\epem\ra\epem$} and \mbox{$\epem\ra\mumu$} (see \Cref{sec:appendix_major_production_cross_sections}),
...
Belle~II consists of several detector subsystems (\Crefrange{sec:pxd}{sec:klm}) arranged cylindrically around the interaction region. 
The visual representation of the Belle~II detector is given in \Cref{fig:belle2_detector} and shows the subdetectors with their acronyms.
\begin{figure}[hbtp!]
...
The \PXD is shown in \Cref{fig:pxd}, whereas its schematic location in the Belle~II detector is depicted in \Cref{fig:vxd}.
...
\begin{figure}[hbtp!]
    ...
    The pixel (\subref{fig:pxd}) and silicon (\subref{fig:svd}) vertex detectors.
...
The grouping of layers is shown in \Cref{fig:cdc_quadrant}, whereas \Cref{fig:cdc_wires} illustrates the difference between axial ad skewed layers.
...
\begin{figure}[hbtp!]
    ...
    In (\subref{fig:cdc_quadrant}), a quadrant of the \CDC in the $r$-$\phi$ plane is shown.
    Different axial and skewed layer groups are visible.
    In (\subref{fig:cdc_wires}), the axial (upper) and skewed (lower) wires are shown.
...
\begin{figure}[hbtp!]
    ...
    \subcaptionbox{\label{fig:arich}}{
        \includegraphics[width=0.45\textwidth]{figures/experimental_setup/arich.png}
    }
    \subcaptionbox{\label{fig:top}}{
        \includegraphics[width=0.45\textwidth]{figures/experimental_setup/top.png}
    }
    \caption{\label{fig:pid}
    The schematic working principle of the Belle~II particle identification detectors: aerogel ring-imaging Cherenkov counter (\subref{fig:arich}) and a time of propagation chamber (\subref{fig:top}).
...
Furthermore, the \ECL provides information necessary to differentiate electrons from hadrons, participates in $K_L^0$ detection together with the \KLM (see \Cref{sec:klm}),
...
\begin{figure}[hbtp!]
    ...
    \caption{\label{fig:klm}
        The Belle~II $K_L^0$ and $\mu$ detector active detector layers in the barrel (\subref{fig:rpc_klm}) and the endcap regions (\subref{fig:scintilator_klm}).
...
\begin{figure}[hbtp!]
    ...
    The definitions of the helix parameters are given in \Cref{sec:tracking}.
...
\begin{figure}[hbtp!]
    ...
    The low-energy deposits resulting from beam background radiation are included in (\subref{fig:clustering1}).
    The cluster reconstruction algorithm of \basftwo singles out the cluster from the photon, 
    rejecting background as seen in (\subref{fig:clustering2}).
