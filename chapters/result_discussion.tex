...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:measurement_comparison}
        The results of the measurement of the total \BtoXsgamma branching fraction compared with past results from other experiments.
...
The hadronic-tagged \BtoXsgamma analysis is dominated statistically as seen in \Cref{tab:integrated_branching_fractions}.
...
Therefore, although individually the result is not competitive with the most precise measurements of BaBar and Belle (note that those measurements are performed with up to 3 times larger datasets),
it shows important consistency between different measurement techniques.
...
The systematic uncertainty highly depends on the lower-\EB threshold due to different components of the systematic uncertainty obtaining different importance.
...
The result of this analysis, evaluated at $1.8$ and $2.0$~\gev thresholds, is linearly interpolated: $0.5\cdot(0.52+0.86)\approx0.69$.
...
On the other hand, if remaining after-fit \BB backgrounds are understood to a $5\%$ level it is plausible to half the expected systematic uncertainty.
...
\section{Input of the results to theory}\label{sec:input_to_theory}
...
The fit results and the comparison of the effect on the total value of the evaluated parameters are shown in \Cref{fig:simba_c7}.
Note that the $\EB\in(1.8,2.0)~\gev$ interval is excluded from the fit due to the large uncertainty and differences in the interval widths compared to earlier \BtoXsgamma measurements.
...
\begin{figure}[htbp!]
    ...
    \Cref{fig:c7} show the corresponding $|C^{\mathrm{incl}}_7V^{}_{tb}V^*_{ts}|$ and $m_b^{1S}$ values.
