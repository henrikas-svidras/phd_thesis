...
As a unitary matrix, it can be fully described by 9 parameters, however, each quark field can absorb a complex phase related to the matrix,
which leaves only a single global complex phase.
Therefore, the \CKM matrix can be expressed in terms of the three quark-mixing angles and $\mathcal{CP}$-violating complex phase.
...
This choice is preferable compared to the $|V_{ub}|$-suppressed and more model-dependant $\B\to X_u\ell\bar{\nu}$.

At the leading-order, where non-perturbative effects are disregarded, effects from $\mathcal{C}_{2,7,8}$ are most important and this can be expressed in a compact form \cite{Buras:1993xp}:
...
Furthermore, having a second Higgs doublet could allow solving the strong CP problem.
...
The decay rate calculation for the \TwoHDM model, with the above-described procedure, 
then proceeds similarly as described in \Cref{sec:btosgamma_totalrate_theory}, by modifying the effective Wilson coefficients based on \Cref{eq:corr_wilson} and calculating appropriate corrections \cite{Ciuchini:1997xe}:
