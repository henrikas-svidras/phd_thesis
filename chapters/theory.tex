...
The Standard Model (\SM) is a quantum field theory based with a local gauge invariance given by the symmetry group $SU(3)\times~SU(2)\times~U(1)$.
...
The Lagrangian, $\mathcal{L}=\mathcal{L}(\psi,\partial_{\mu}\psi)$, describes the dynamics originating from the excitations of field $\phi$,
...
The spin-0 Higgs boson couples to all massive gauge bosons of the \SM via the Higgs mechanism \cite{PhysRevLett.13.508} and to fermions via the Yukawa couplings~\cite{Weinberg:1967t}.
...
\begin{figure}[htbp!]
    \centering
    \input{tikz_diagrams/standard_model.tex}
    ...
    See in Ref.~\cite{Workman:2022ynf} for more information on the values.
...
Each species of fermion has a distinct \textit{flavour}, resulting in six quark and six lepton flavours in \SM.
The strong and electromagnetic interaction do not change the flavour of particles, 
...
The combinations of a quark and an anti-quark pair are called \textit{mesons}, while a combination of three (anti-)quarks are called \textit{baryons}.
...
\begin{table}[htbp!]
    ...
    \input{tables/hadron_examples.tex}
\end{table}
...
In the \SM, each particle has a partner particle, called an \textit{anti-particle}.
For a particle $\psi$, an anti-particle can be obtained by performing the charge conjugation transformation:
\begin{equation}
...
The anti-particles of neutrinos ($\nu_{e},\nu_{\mu},\nu_{\tau}$) are denoted as ($\bar{\nu}_{e},\bar{\nu}_{\mu},\bar{\nu}_{\tau}$)
...
it was initially assumed thought that their combination, $\mathcal{CP}$ symmetry is conserved.
...
In order to incorporate $\mathcal{CP}$ violation into the \SM, M. Kobayashi and T. Maskawa~\cite{Kobayashi:1973fv} built upon the predecessor work by N. Cabibo \cite{Cabibbo:1963yz},
...
Kobayashi's and Maskawa's work extended this and showed that at least three generations of quarks is required 
...
The decay of the unstable $b$ quark always contains a flavour changing processes via the weak interaction.
Therefore, the study of their decays open the possibility to measure a wide range of parameters of the \SM, including the \CKM matrix, $\mathcal{CP}$ violation measurements, lepton mass measurements etc.
...
To exploit the properties of $B$ meson decays, special type of experiments, known as \textit{$B$ factories}, are designed to study $B$ mesons.
...
One of the decay channels used to study these transitions are the so-called rare radiative $b\ra s\g$ and $b\ra d\g$ decays.
...
The latter point does not hold for $\b\to\d$ case, where the $V^*_{uq}V_{ub}/V^*_{tq}V_{tb}$ multiplied term is not numerically small and contributes already at leading order.
...
As long as an appropriately low photon energy threshold (in the decaying $B$ meson rest frame), \EB, is chosen, the non-perturbative effects in \Cref{eq:btosgammarate_nonp} can be considered smaller as they `average out' over the spectrum.
...
which is defined in the Appendix A of Ref.~\cite{Buras:1993xp}.
...
In the absence of \QCD effects, one has $\eta=1$ and \BtoXsgamma decays are governed solely by the photonic dipole exchange.
...
\end{equation}
\sloppy For $\BtoXdgamma$, as evident from \Cref{eq:effective_lagrangian}, the result is suppressed by a factor \mbox{$|V_{td}/V_{ts}|^2\approx4.2\%$} with respect to the \BtoXsgamma result.
