...
\begin{figure}[htbp!]
    ...
    \caption{\label{fig:xsgamma_theory_sketches} \todo[inline]{Fill}. Credit to Frank Tackmann.}
\end{figure}
...
\end{equation}
As the leading order \BtoXsgamma is closely approximated as a two-body decay, it is clear that the peak region (close to $m_b/2$) is governed strongly by the shape-function.
In that case $|C_7|$ just governs the normalisation of the spectrum.
...
\end{equation}
\Cref{eq:moments_of_shape_function} are the equations for the $n$th moments of the shape function.
...
Therefore, for total decay-rate calculations (such as the ones sketched in \Cref{sec:btosgamma_totalrate_theory}) it is enough to know the first several moments of the shape function, as the more delicate $\mathcal{F}(k)$ dependance is integrated.

Conversely to the Wilson coefficients, the form of the shape function is not well-known and non-perturbative.
...
The shape function is modelled using a simple form:
\begin{equation}
...
Therefore, the shape-function can be fully described by two free parameters: the $b$-quark pole-mass $m_b$ and the residual-momentum of the $b$-quark in the $B$ meson, $\lambda_1$.
This relatively straightforward relation and the fact that the Kagan-Neubert model is readily available in \texttt{EvtGen} Monte Carlo generator, commonly used by $B$-factories \cite{Ryd:2005zz}, made it the most common choice for experimental description of the \Egamma spectrum.
...
We generate the spectrum for the experimental analysis using the evualuated values of $m_b$ and $\lambda_1$ by the SIMBA collaboration \cite{Bernlochner:2020jlt}:

\begin{equation}
    values.
\end{equation}

\todo[inline]{Need to tie in renormalisation scheme somehow and say that we convert values from SIMBA}

\section{New-physics opportunities in \BtoXsgamma}
