...
and $\Delta\Gamma_{non-p.}$ is the non-perturbative contribution arising outside of local operator product expansion when accounting for the fact that the $b$ quark is not stationary inside the bound $B$ meson state.
...
\begin{figure}[hbtp!]
    ...
    In (\subref{fig:theory_to_experiment}), the leading-order $\delta$ function spectrum, the non-perturbative shape function effects, and the convolution of these effects with the perturbatively calculable ones are represented.
    It also includes the experimental effects, such as finite detector resolution.
    In (\subref{fig:theory_ingredients_schematic}), the origin of perturbative and non-perturbative effects in a \BtoXsgamma decay are illustrated.
    Credit to Dr. Frank Tackmann for the Figures. 
    The data points in (\subref{fig:theory_to_experiment}) correspond to the Belle measurement in \cite{Belle:2009nth}.}
\end{figure}
...
\begin{figure}[hbtp!]
    ...
    In (\subref{fig:knmodel_mb_variation}), variations of $m_b$ affecting the spectrum while keeping $\lambda_1$ constant are shown.
    In (\subref{fig:knmodel_lambda_variation}), the inverse scenario is depicted.
...
\begin{figure}[hbtp!]
    ...
    In (\subref{fig:xsgamma_2hdm_type1}), the dependence for a type-I \TwoHDM model is shown, whereas (\subref{fig:xsgamma_2hdm_type2}) shows that for type-II \TwoHDM.
