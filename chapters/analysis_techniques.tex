...
However, it requires a good knowledge of the form of the \PDF $f(x;a)$ that the data follows. 
...
Assuming \Cref{eq:likelihood_derivative} is satisfied, the log-likelihood function can be expanded as a series at $a=\hat{a}$ and approximated as a following parabola:
\begin{equation}
...
This is only acceptable in the case of large training samples, where statistical fluctuations of data are negligible, making the validation and testing samples statistically equivalent.
...
\begin{figure}[htbp!]
    ...
    Two-dimensional distributions (red/green) in an example feature space are shown in \Cref{fig:distributions_separation} for four different example datasets (0-3).
    ...
    The working points that represent the dashed lines of \Cref{fig:distributions_separation} are shown in the same colour, for each of the four curves.
...
When combining many features together, selecting an appropriately parametrised model (\Cref{eq:loss_function}) can be complicated and suboptimal.
...
With pruning, their performance on independent samples usually turns out to be weaker than many other \MVA{s}, and the misclassification rate only slightly better than random guesses.
...
Boosted Decision Trees (\BDT{s}) are often considered as `black box' models because their inner structure is
...
\end{equation}
There are a number of methods to solve \Cref{eq:unfolding_linear_equation}, discussed in the following sections.
...
Regularisation is a delicate task because, roughly put, the scientist has a prejudice towards smooth measured distributions and therefore artificially adds a bias to smoothen them.
Unsurprisingly, there exist many methods to perform regularisation.
Although an in-depth discussion is irrelevant for the purpose of the work presented in this thesis, some of the most common ones in particle physics are listed here:
\begin{itemize}
