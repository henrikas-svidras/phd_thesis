...
They are compiled in \Cref{tab:compilation_results}.
...
\begin{table}[hbtp!]
    ...
    \begin{tabular}{|l|cc|}
        \hline
        Observable [$\EB>1.8~\gev$] & Results of the analysis & World average value~\cite{Workman:2022ynf}\\ 
        % & Standard Model prediction \\
        \hline
        $\mathcal{B}(\BtoXsgamma)$ [$10^{-4}$]& 
        $3.54 \pm 0.78$ (stat.) $\pm~0.83$ (syst.)  &
        $3.49\pm0.19$ 
        % $3.40\pm0.17$~\cite{Misiak:2020vlo} 
        \\

        $\expval**{\EB}$ [\gev] & 
        $2.284 \pm 0.065$ (stat.) $\pm 0.071$ (syst.) &
        $2.314 \pm 0.011$ 
        % $\approx 2.312$~\cite{Benson:2004sg} (model dependent)
        \\

        $\expval**{\EB^2}-\expval**{\EB}^2$ [$\gev^2$] & 
        $0.0502 \pm 0.0157$ (stat.) $\pm 0.0176$ (syst.) &
        $0.0303 \pm 0.0025$ 
        % $\approx 0.0309$~\cite{Benson:2004sg} (model dependent)        
        \\
        \hline
    \end{tabular}
    }
